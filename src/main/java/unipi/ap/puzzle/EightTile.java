...
public class EightTile extends JButton implements PropertyChangeListener, Restart.RestartListener {
    ...
    public EightTile() {
        ...
        this.addActionListener(e -> {
            // When the button is clicked, fire a VETOABLE_MOVE
            try {
                // The propagation id is the tile itself
                super.fireVetoableChange("VETOABLE_MOVE", this.label, 9);
    ...
    public void propertyChange(PropertyChangeEvent e) {
        // If the event is a label update event and the label of the tile is equal to
        // the new value of the event
        if (e.getPropertyName() == "UPDATE_LABEL") {
